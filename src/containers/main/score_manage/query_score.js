...
class QueryScore extends React.Component {
  ...
  render(){
    ...
    const columns = [{
      ...
      render: (text, record) => (
        <span>
          <Button size="small" onClick={this.changeClass.bind(this,record)}>查看</Button>
        </span>
      ),
    }, {
      title: '学年',
    ...
    return(
      <div>
        <BreadcrumbCustom pathList={['成绩管理','查询成绩']}></BreadcrumbCustom>
        <div className="class-manage-content">
          <Row gutter={8}>
            <Col  {...ColProps} >
              <Button type="primary"  onClick={this.showAllClass.bind(this)}>所有学生</Button>
            </Col>  
            <Col  {...ColProps} >
              <Search
